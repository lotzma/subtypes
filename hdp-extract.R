...
#Load data set(s)
mut_data <- read.csv("data/tcga_all_sorted.csv", row.names=1)

group_size <- c(167, 88, 23, 52)

# initialise HDP, assign data, activate
hdp <- hdp_init(ppindex = c(0, 1, 1, 1, 1, rep(2, group_size[1]), rep(3, group_size[2]), rep(4, group_size[3]), rep(5, group_size[4])),
                cpindex = c(1, 2, 2, 2, 2, rep(3, group_size[1]), rep(4, group_size[2]), rep(5, group_size[3]), rep(6, group_size[4])),
                hh = rep(1, 96),
                alphaa = rep(1, 6),
                alphab = rep(1, 6))

hdp <- hdp_setdata(hdp, 6:numdp(hdp), mut_data)
...
png(filename="exp_W3.png", width=900, height=600)
plot_dp_comp_exposure(hdp_multi, 5+group_size[1]+group_size[2]+group_size[3] + 1:group_size[4],
                      RColorBrewer::brewer.pal(12, "Set3"),
                      incl_numdata_plot=TRUE, incl_nonsig=TRUE,
                      main_text="Triple wild", ylab_numdata="SNV count",
                      ylab_exp="Signature exposure",
                      leg.title="Signature", cex.axis = 1)
dev.off()
...
plot_dp_comp_exposure(hdp_multi, 2:5,
                      RColorBrewer::brewer.pal(12, "Set3"),
                      incl_numdata_plot=FALSE,
                      main_text="Group signature distribution",
                      ylab_exp="Signature exposure",
                      leg.title="Signature", cex.axis = 1,
                      dpnames=c("BRAF", "NRAS", "NF1", "W3"),
                      cex.names=0.8)
dev.off()
...
w3_group_ci <- comp_dp_distn(hdp_multi)$cred.int[[5]]
...
write.table(w3_group_ci, file="w3_group_credibility_intervals.txt",
            row.names=FALSE, sep="\t")
